  ...
  pub(crate) fn lower_expr(&mut self, ast: Option<ast::Expr>) -> Expr {
    if let Some(ast) = ast {
      match ast {
        ...
        ast::Expr::Number(ast) => Expr::Number { n: ast.parse() },
...
mod tests {
  ...
  #[test]
  fn lower_expr_stmt() {
    check_stmt("123", Stmt::Expr(Expr::Number { n: Some(123) }));
  }

  #[test]
  fn lower_binary_expr() {
    let mut exprs = Arena::new();
    let lhs = exprs.alloc(Expr::Number { n: Some(1) });
    let rhs = exprs.alloc(Expr::Number { n: Some(2) });
  ...
  fn lower_binary_expr_without_rhs() {
    let mut exprs = Arena::new();
    let lhs = exprs.alloc(Expr::Number { n: Some(10) });
  ...
  #[test]
  fn lower_literal() {
    check_expr("999", Expr::Number { n: Some(999) }, Database::default());
  }
  ...
  fn lower_unary_expr() {
    let mut exprs = Arena::new();
    let ten = exprs.alloc(Expr::Number { n: Some(10) });
